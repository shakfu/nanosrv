...
- **DNS ignored the host's resolver.** `mgr_init` hardcoded `udp://8.8.8.8:53`, so outbound resolution never consulted `/etc/resolv.conf`: internal names silently failed, split-horizon DNS was bypassed, container and VPN resolvers were ignored, and every query left the host. The first `nameserver` entry of each family is now read at startup, with the old constants kept as a fallback for when the file names none (and for Windows, which has no such file). The parser takes the file contents rather than a path, so it is covered by tests that do not depend on the host's configuration

- **`-DMG_ENABLE_IO_URING=1` did not work, and io_uring auto-enabled itself.** The backend was selected by `__has_include(<liburing.h>)`, so the poller depended on whether a development package happened to be installed -- the same source and compiler could produce different builds on different machines. Setting the macro explicitly was worse: it skipped the io_uring branch of the `#elif` chain and fell through to the epoll one, enabling both backends and tripping the mutual-exclusion `static_assert`. io_uring is now opt-in and selected correctly, and a build that requests it without liburing available fails with a clear `#error` instead of an obscure one

- **The interpreter printed a leak report on normal exit.** Any nanosrv object alive at shutdown -- a module-level `Manager` stopped with Ctrl-C, exactly as the README shows -- made nanobind print "leaked N instances! ... likely caused by a reference counting issue". Objects held to the end of the process are not leaks. The warnings are off by default and restored with `NANOSRV_LEAK_WARNINGS=1` for anyone actually chasing a reference-counting bug
