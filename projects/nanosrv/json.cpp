...
static int pass_string(const char* s, int len)
{
    int i;
    for (i = 0; i < len; i++) {
...
static double atod(const char* p, int len, int* numlen)
{
    double d = 0.0;
    int i = 0, sign = 1;
    ...
    if (i < len && (p[i] == 'e' || p[i] == 'E')) {
        ...
        if (i < len && p[i] == '+')
            i++;
        while (i < len && p[i] >= '0' && p[i] <= '9' && exp < MG_DOUBLE_MAX_EXPONENT)
            exp = exp * 10 + (p[i++] - '0');
        // use fast exponentiation
        // https://en.wikipedia.org/wiki/Exponentiation_by_squaring
        if (exp != 0) {
            double x = 10, y = 1;
            if (exp > MG_DOUBLE_MAX_EXPONENT)
                exp = MG_DOUBLE_MAX_EXPONENT;
            if (minus)
                x = 0.1;
            while (exp > 1) {
                if (exp & 1) {
                    ...
                    struct Str* val)
{
    if (ofs >= obj.len) {
        ...
        struct Str sub = str_n(obj.buf + ofs, obj.len - ofs);
        if (ofs == 0)
            ofs++, sub.buf++, sub.len--;
        if (*obj.buf == '[') { // Iterate over an array
            int n = 0, o = json_get(sub, "$", &n);
            if (n < 0 || o < 0 || static_cast<size_t>(o + n) > sub.len) {
                ofs = 0; // Error parsing key, stop scanning
            } else {
                if (key)
                    *key = str_n(NULL, 0);
                if (val)
                    *val = str_n(sub.buf + o, static_cast<size_t>(n));
                ofs = static_cast<size_t>(&sub.buf[o + n] - obj.buf);
            }
            ...
            if (n < 0 || o < 0 || static_cast<size_t>(o + n) > sub.len) {
                ...
                if (key)
                    *key = str_n(sub.buf + o, static_cast<size_t>(n));
                sub.buf += o + n, sub.len -= static_cast<size_t>(o + n);
                while (sub.len > 0 && *sub.buf != ':')
                    sub.len--, sub.buf++;
                if (sub.len > 0 && *sub.buf == ':')
                    sub.len--, sub.buf++;
                n = 0, o = json_get(sub, "$", &n);
                if (n < 0 || o < 0 || static_cast<size_t>(o + n) > sub.len) {
...
int json_get(struct Str json, const char* path, int* toklen)
{
    ...
    int len = static_cast<int>(json.len);
...
#define MG_EOO(x)                                                             \
    ...
    for (i = 0; i < len; i++) {
        unsigned char c = (reinterpret_cast<const unsigned char*>(s))[i];
        if (c == ' ' || c == '\t' || c == '\n' || c == '\r')
            continue;
        switch (expecting) {
        case S_VALUE:
            ...
            if (depth == ed)
                j = i;
            if (c == '{') {
                if (depth >= static_cast<int>(sizeof(nesting)))
                ...
                int numlen = 0;
                atod(&s[i], len - i, &numlen);
                i += numlen - 1;
            } else if (c == '"') {
                int n = pass_string(&s[i + 1], len - i - 1);
                if (n < 0)
                    return n;
                i += n + 1;
            } else {
        ...
        case S_KEY:
            if (c == '"') {
                ...
                if (depth == ed && path[pos - 1] == '.'
                    && strncmp(&s[i + 1], &path[pos], static_cast<size_t>(n)) == 0
...
{
    int len = 0, ofs = json_get(json, path, &len);
    return str_n(ofs < 0 ? NULL : json.buf + ofs,
                    (size_t)(len < 0 ? 0 : len));
}
...
bool json_get_num(struct Str json, const char* path, double* v)
{
    ...
    if ((n = json_get(json, path, &toklen)) >= 0
        && (json.buf[n] == '-'
            || (json.buf[n] >= '0' && json.buf[n] <= '9'))) {
        if (v != NULL)
            *v = atod(json.buf + n, toklen, NULL);
        found = 1;
    }
    return found;
}
...
{
    int found = 0, off = json_get(json, path, NULL);
...
bool json_unescape(struct Str s, char* to, size_t n)
{
    size_t i, j;
    for (i = 0, j = 0; i < s.len && j < n; i++, j++) {
        if (s.buf[i] == '\\' && i + 5 < s.len && s.buf[i + 1] == 'u') {
            ...
            if (str_to_num(str_n(s.buf + i + 2, 4), 16, &to[j],
                              sizeof(uint8_t))
                == false)
                return false;
            i += 5;
...
{
    char* result = NULL;
    int len = 0, off = json_get(json, path, &len);
    if (off >= 0 && len > 1 && json.buf[off] == '"') {
        if ((result = static_cast<char*>(mem_calloc(1, static_cast<size_t>(len)))) != NULL
            && !json_unescape(
                str_n(json.buf + off + 1, static_cast<size_t>(len - 2)), result,
...
char* json_get_b64(struct Str json, const char* path, int* slen)
{
    char* result = NULL;
    int len = 0, off = json_get(json, path, &len);
    if (off >= 0 && json.buf[off] == '"' && len > 1
        && (result = static_cast<char*>(mem_calloc(1, static_cast<size_t>(len)))) != NULL) {
        size_t k = base64_decode(json.buf + off + 1, static_cast<size_t>(len - 2),
                                    result, static_cast<size_t>(len));
        if (slen != NULL)
            *slen = static_cast<int>(k);
    }
    return result;
}

char* json_get_hex(struct Str json, const char* path, int* slen)
{
    char* result = NULL;
    int len = 0, off = json_get(json, path, &len);
    if (off >= 0 && json.buf[off] == '"' && len > 1
        && (result = static_cast<char*>(mem_calloc(1, static_cast<size_t>(len) / 2))) != NULL) {
        int i;
        for (i = 0; i < len - 2; i += 2) {
            str_to_num(str_n(json.buf + off + 1 + i, 2), 16,
...
long json_get_long(struct Str json, const char* path, long dflt)
{
    double dv;
    long result = dflt;
    if (json_get_num(json, path, &dv))
        result = static_cast<long>(dv);
    return result;
}

// -- Modern C++ API: json namespace convenience functions --

namespace json {

std::optional<double> number(std::string_view json, const char* path)
{
    struct Str s = str_n(json.data(), json.size());
    double v = 0;
    if (json_get_num(s, path, &v))
        return v;
    return std::nullopt;
}
